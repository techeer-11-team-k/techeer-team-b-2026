...
현재 아키텍처는 EC2에서 백엔드와 모니터링을 함께 운영하고,
RDS/ElastiCache는 관리형 서비스로 사용합니다. S3는 정적 파일 저장용이지만 현재는 미사용입니다.
...
## 🛠️ 방법 2: EC2 + Docker Compose 배포 (현재 사용)
...
### 4단계: 모니터링 구성 (Prometheus/Grafana)

EC2에서 백엔드와 함께 모니터링 스택을 실행합니다. Docker Compose에
Prometheus/Grafana 서비스를 포함하고, 필요한 포트를 보안 그룹에 추가합니다.

- Prometheus: `9090`
- Grafana: `3000`

실제 포트 오픈은 내부망 또는 VPN 사용을 권장합니다.

### 5단계: RDS 및 ElastiCache 연결
...
정적 파일을 저장해야 할 경우에만 S3 버킷을 생성해 사용합니다.
현재 서비스에서는 S3 사용 항목이 없으므로 생성은 선택 사항입니다.
