...
export const ApartmentRow: React.FC<ApartmentRowProps> = ({
  ...
  const baseClasses = `group flex flex-col md:flex-row md:items-center justify-between border-b border-slate-100 last:border-0 transition-all duration-200 cursor-pointer ${variant === 'default' ? 'rounded-2xl md:rounded-xl hover:bg-slate-50 active:scale-[0.98]' :
    ...
    >
      <div className="flex items-start md:items-center gap-4 flex-1 min-w-0 w-full">
        ...
        {showRank && rank !== undefined && (
          <div className={`flex-shrink-0 w-8 h-8 md:w-10 md:h-10 rounded-xl flex items-center justify-center font-black mt-1 md:mt-0 ${isTop3
            ? rank === 1
            ...
            <div className="flex justify-between items-center">
              ...
              <div className="md:hidden text-right pl-2">
                {mobileRightContent ? mobileRightContent : (
                  <p className={`font-bold text-[16px] tabular-nums tracking-tight ${isVisible !== false ? 'text-slate-900' : 'text-slate-400'
                    }`}>
        ...
        {rightContent || (
          <>
            <div className="text-right min-w-0">
              <p className={`font-bold text-[15px] md:text-[17px] tabular-nums tracking-tight truncate ${isVisible !== false ? 'text-slate-900' : 'text-slate-400'
                }`}>
                <FormatPrice value={price} />
              </p>
              {showChangeRate && changeRate !== undefined && (
                <p className={`text-[12px] md:text-[13px] mt-0.5 font-bold tabular-nums whitespace-nowrap ${changeRate >= 0 ? 'text-red-500' : 'text-blue-500'
                  }`}>
