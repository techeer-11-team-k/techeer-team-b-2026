...
export const Dashboard: React.FC<ViewProps> = ({ onPropertyClick, onViewAllPortfolio, onSettingsClickRef }) => {
    ...
    // 온보딩 직후 대시보드 진입 시, 방금 등록한 자산/관심 단지를 강조하는 오버레이
    const [onboardingOverlay, setOnboardingOverlay] = useState<{
        mode: 'my' | 'favorites' | null;
        aptId?: number | null;
        aptName?: string | null;
    } | null>(null);
    const [isOnboardingOverlayVisible, setIsOnboardingOverlayVisible] = useState(false);
    const [isOnboardingStartReady, setIsOnboardingStartReady] = useState(false);
    ...
    // 온보딩 완료 직후 대시보드 진입 시 1회성 오버레이 표시
    useEffect(() => {
        if (!isClerkLoaded || !isSignedIn) return;
        try {
            const raw = window.localStorage.getItem('onboarding.dashboardOverlay');
            if (!raw) return;
            ...
            setOnboardingOverlay({
                ...
                aptId:
                    typeof parsed.aptId === 'number'
                        ? parsed.aptId
                        : parsed.aptId != null
                            ? Number(parsed.aptId)
                            : null,
                aptName: parsed.aptName ?? null,
            });
            setIsOnboardingOverlayVisible(true);
            window.localStorage.removeItem('onboarding.dashboardOverlay');
        } catch {
            // ignore
        }
    }, [isClerkLoaded, isSignedIn]);

    // "시작하기" 버튼은 250ms 후 서서히 나타나도록
    useEffect(() => {
        if (!isOnboardingOverlayVisible) {
            setIsOnboardingStartReady(false);
            return;
        }
        const t = window.setTimeout(() => {
            setIsOnboardingStartReady(true);
        }, 250);
    ...
    const ControlsContent = () => (
        <>
            ...
            <div className="flex items-center gap-2 mb-6 border-b border-slate-100 pb-3">
                {/* 스크롤 가능한 탭 영역 */}
                <div className={`flex items-center gap-2 flex-1 ${assetGroups.length >= 3
                    ...
                    {assetGroups.map((group) => (
                        <div
                            ...
                            className={`relative flex items-center gap-1 flex-shrink-0 ${draggedGroupId === group.id ? 'opacity-50' : ''
                        ...
                        >
                            {isEditMode && editingGroupId === group.id ? (
                                ...
                                <button
                                    ...
                                    className={`px-4 py-2 rounded-lg text-[15px] font-bold transition-all whitespace-nowrap border min-w-[80px] text-center ${activeGroupId === group.id
    ...
    if (isSignedIn && !isOnboardingCompleted) {
        ...
        try {
            const skipAll = window.localStorage.getItem('onboarding.skipAll');
            if (skipAll === '1') {
                shouldBypass = true;
            }
                                                            ...
                                                            {[
                                                                ...
                                                                >
                                                                    ...
                                                                    <div className="text-right">
                                                                        ...
                                                                        <div
                                                                            className={`text-[11px] font-black tabular-nums ${r.diff.startsWith('+') ? 'text-red-500' : 'text-blue-500'
                                                                                }`}
                                                                        >
                                                                            {r.diff}
                                                                        </div>
    ...
    return (
        <div className="relative">
            {isOnboardingOverlayVisible && (
                <div className="fixed inset-0 z-[90] flex items-center justify-center bg-slate-950/40 backdrop-blur-md">
                    <div className="w-full max-w-md rounded-3xl bg-white/95 border border-slate-200 shadow-2xl px-6 py-6 md:px-7 md:py-7">
                        <div className="space-y-4">
                            <div className="space-y-1.5">
                                <div className="text-[13px] font-semibold tracking-[0.16em] uppercase text-slate-500">
                                    온보딩 완료
                                </div>
                                <div className="text-[22px] md:text-[24px] font-black text-slate-900 leading-snug">
                                    설정이 완료되었어요.
                                </div>
                                <div className="text-[13px] md:text-[14px] text-slate-600">
                                    {onboardingOverlay?.mode === 'my'
                                        ? '방금 등록한 내 자산의 가격 흐름과 변화를 먼저 보여드릴게요.'
                                        : onboardingOverlay?.mode === 'favorites'
                                            ? '방금 추가한 관심 단지의 가격 흐름과 변화를 먼저 보여드릴게요.'
                                            : '이제 대시보드에서 자산과 관심 단지의 흐름을 한 번에 볼 수 있어요.'}
                                </div>
                            </div>

                            {/* 간단한 그래프 느낌의 애니메이션 블록 */}
                            <div className="mt-2 rounded-2xl border border-slate-100 bg-slate-50/80 px-4 py-3">
                                <div className="flex items-end gap-1.5 h-20">
                                    {[0.4, 0.7, 0.5, 0.9, 0.6].map((base, idx) => (
                                        <motion.div
                                            // eslint-disable-next-line react/no-array-index-key
                                            key={idx}
                                            className="flex-1 rounded-full bg-gradient-to-t from-slate-300 to-slate-500/90"
                                            initial={{ scaleY: base, opacity: 0.8 }}
                                            animate={{ scaleY: [base, base + 0.25, base + 0.05, base + 0.2, base] }}
                                            transition={{
                                                duration: 1.8,
                                                repeat: Infinity,
                                                ease: 'easeInOut',
                            ...
                            <div className="mt-3 flex justify-end">
                                <button
                                    ...
                                    className={`inline-flex items-center justify-center rounded-2xl bg-slate-900 px-5 py-2.5 text-[14px] font-bold text-white shadow-md shadow-slate-900/15 transition-opacity duration-200 ${isOnboardingStartReady ? 'opacity-100' : 'opacity-0 pointer-events-none'
                                        }`}
                                >
                                    시작하기
                                </button>
            ...
            {toast && (
                <div className="fixed top-20 left-1/2 -translate-x-1/2 z-[200] animate-slide-down">
                    <div className={`px-4 py-3 rounded-xl shadow-2xl flex items-center gap-3 min-w-[300px] ${toast.type === 'success'
                                    ...
                                    <div className="bg-white rounded-[28px] p-10 shadow-[0_2px_8px_rgba(0,0,0,0.04)] border border-slate-100/80 flex flex-col h-full min-h-0 relative">
                                        <div className="flex items-center justify-between mb-6 px-1">
                                            ...
                                            <button
                                                ...
                                                className={`text-[13px] font-bold flex items-center gap-1.5 p-2 rounded-lg transition-colors ${isEditMode
                            ...
                            <div className="flex gap-2 mb-4">
                                {['1년', '3년', '전체'].map(t => (
                                    <button
                                        key={t}
                                        onClick={() => setSelectedPeriod(t)}
                                        className={`text-[11px] font-bold px-3 py-1.5 rounded-full transition-all ${t === selectedPeriod
                                ...
                                <div className="space-y-2">
                                    {assetGroups.map((group) => (
                                        <button
                                            ...
                                            className={`w-full flex items-center justify-between p-4 rounded-xl transition-all ${activeGroupId === group.id
                                                ? 'bg-blue-50 border-2 border-blue-500'
                                                : 'bg-slate-50 border-2 border-transparent hover:bg-slate-100'
                                                }`}
                                        >
                                            <div className="flex items-center gap-3">
                                                <div className={`w-10 h-10 rounded-full flex items-center justify-center ${activeGroupId === group.id ? 'bg-blue-500' : 'bg-slate-300'
                                                    }`}>
                                                    <Layers className={`w-5 h-5 ${activeGroupId === group.id ? 'text-white' : 'text-slate-600'
                                                        }`} />
                                                </div>
                                                <div className="text-left">
                                                    <p className={`text-[15px] font-bold ${activeGroupId === group.id ? 'text-blue-600' : 'text-slate-900'
                                                        }`}>
                                                        {group.name}
                                                    </p>
                            ...
                            <div className="bg-white rounded-[20px] p-5 shadow-sm border border-slate-100">
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    <button
                                        ...
                                        className={`p-4 rounded-xl border-2 transition-all ${viewMode === 'separate'
                                            ? 'border-blue-500 bg-blue-50'
                                            : 'border-slate-200 hover:border-slate-300'
                                            }`}
                                    >
                                        <div className={`text-[14px] font-bold ${viewMode === 'separate' ? 'text-blue-600' : 'text-slate-900'
                                            }`}>
                                            개별 보기
                                        </div>
                                    ...
                                    <button
                                        ...
                                        className={`p-4 rounded-xl border-2 transition-all ${viewMode === 'combined'
                                            ? 'border-blue-500 bg-blue-50'
                                            : 'border-slate-200 hover:border-slate-300'
                                            }`}
                                    >
                                        <div className={`text-[14px] font-bold ${viewMode === 'combined' ? 'text-blue-600' : 'text-slate-900'
                                            }`}>
                                            모아 보기
                                        </div>
                                    ...
                                    {activeGroup.assets.map(asset => (
                                        ...
                                        >
                                            ...
                                            <button
                                                ...
                                                className={`p-2 rounded-lg transition-colors ${asset.isVisible
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                    {[
                                        ...
                                        <button
                                            ...
                                            className={`p-3 rounded-xl text-[13px] font-bold transition-all ${sortOption === option.value
