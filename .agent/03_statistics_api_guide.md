...
임시 구현 방안(3.6)은 무시하고 바로 방안 2를 적용하여 새로운 테이블과 API를 개발하라
...
수정 제안: 3.7 구현 시 주의사항 (변경분)
3. 지역 필터링 및 데이터 추출 로직 (중요):

region_type 파라미터는 '출발 지역(from_region)'의 소속을 결정하는 기준임.

전국: 모든 from_region을 대상으로 하되, 이동량(movement_count) 기준 상위 N개(예: 20개)의 흐름을 반환

수도권: from_region이 [서울, 경기, 인천] 중 하나인 모든 이동 데이터를 반환. (이때 to_region은 전국 어디든 포함될 수 있음)

지방5대광역시: from_region이 [부산, 대구, 광주, 대전, 울산] 중 하나인 모든 이동 데이터를 반환.

자기 지역 이동 제외: from_region_id == to_region_id인 데이터는 Sankey Diagram의 가독성을 위해 기본적으로 제외하고 집계할 것.
