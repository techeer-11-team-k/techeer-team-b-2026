...
부동산 서비스에서 **가격은 가장 중요한 정보**이다. "3억 4,000만원"을 동일 스타일로 표시하지 말고, **숫자와 단위를 구분**하여 가독성을 극대화한다.

-   **숫자(3, 4000)**: 크고 굵게, 강조 색상 (`text-slate-900` / `text-slate-100`)
-   **단위(억, 만원)**: 작고 가볍게, 서브 색상 (`text-slate-500` / `text-slate-400`)
-   **변동률**: 가격 바로 아래/옆에, 방향에 맞는 색상 (상승=빨강, 하락=파랑)
...
-   ✅ **카드를 쓸 때**: 독립적 정보 단위, 클릭 가능한 요소, 작은 정보 그룹핑
-   ❌ **카드를 쓰지 않을 때**: 페이지 전체 감싸기, 카드 안에 카드 중첩, 리스트 아이템마다 개별 카드
-   **대안**: 구분선, 여백, 배경색 변화로 영역 구분
