...
class PortfolioPropertyResponse(BaseModel):
    ...
    # 매입 정보
    purchase_date: Optional[date] = Field(None, description="매입일")
    purchase_price: Optional[int] = Field(None, description="매입가격 (만원)")
    ...
    profit_amount: Optional[int] = Field(None, description="수익금 (만원, 양수면 수익, 음수면 손실)")
    holding_period_months: Optional[int] = Field(None, description="보유 기간 (개월)")
    
    model_config = ConfigDict(from_attributes=True)


class PortfolioListResponse(BaseModel):
    """포트폴리오 성과 분석 목록 응답 스키마"""
    properties: list[PortfolioPropertyResponse] = Field(..., description="보유 아파트 목록")
