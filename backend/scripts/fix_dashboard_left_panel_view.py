#!/usr/bin/env python3
"""
dashboard_left_panel_view 컬럼 추가 스크립트

이 스크립트는 accounts 테이블에 dashboard_left_panel_view 컬럼이 없을 때
자동으로 추가합니다.

사용법:
    # Docker 컨테이너에서 실행
    docker-compose exec backend python /app/scripts/fix_dashboard_left_panel_view.py
    
    # 로컬에서 실행
    python backend/scripts/fix_dashboard_left_panel_view.py
"""
import asyncio
import sys
...
async def check_column_exists(engine):
    ...
    async with engine.connect() as conn:
        result = await conn.execute(text("""
            ...
            AND column_name = 'dashboard_left_panel_view'
        """))
...
async def add_column(engine):
    ...
    async with engine.begin() as conn:
        ...
        # 컬럼 코멘트 추가
        await conn.execute(text("""
            COMMENT ON COLUMN accounts.dashboard_left_panel_view IS 
            '대시보드 하단 좌측 카드 뷰 (policyNews|transactionVolume|marketPhase|regionComparison)'
        """))
...
async def fix_column():
    """컬럼 추가 스크립트 실행"""
    print("=" * 60)
    print(" dashboard_left_panel_view 컬럼 추가 스크립트")
    print("=" * 60)
